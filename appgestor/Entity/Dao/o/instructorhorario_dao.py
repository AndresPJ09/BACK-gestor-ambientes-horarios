...
class InstructorHorarioDAO:
    ...
    def obtener_horario_por_usuario(usuario_id):
        """Consulta los horarios filtrados por usuario_id."""
        query = """
            SELECT 
                IH.id AS instructor_horario_id,
                IH.observaciones AS instructor_observaciones,
            ...
            LEFT JOIN sena.NivelFormacion AS N ON PR.nivel_formacion_id_id = N.id
            LEFT JOIN sena.Ambiente AS A ON H.ambiente_id_id = A.id 
            WHERE H.usuario_id_id = %s AND IH.fechaElimino IS NULL 
        """
    ...
    def obtener_horario_por_periodo(periodo_id):
        """Consulta los horarios filtrados por periodo_id."""
        query = """
            SELECT 
                IH.id AS instructor_horario_id,
                IH.observaciones AS instructor_observaciones,
            ...
            LEFT JOIN sena.NivelFormacion AS N ON PR.nivel_formacion_id_id = N.id
            LEFT JOIN sena.Ambiente AS A ON H.ambiente_id_id = A.id 
            WHERE H.periodo_id_id = %s AND IH.fechaElimino IS NULL 
        """

        with connection.cursor() as cursor:
            cursor.execute(query, [periodo_id])
            columns = [col[0] for col in cursor.description]
            results = [dict(zip(columns, row)) for row in cursor.fetchall()]
